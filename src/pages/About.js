...
export default function About({ mobile }) {
    return (
        <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", width: "100%", paddingX: "10vw" }}>
            <TitleCard image={'leaves.png'} mobile={mobile}>
                <Typography sx={{ color: "black" }} variant={(mobile)?"h2":"h1"} >About</Typography>
                <Typography variant="h5" color={"black"}>The paths that I had taken and who I am now</Typography>
            </TitleCard>

            <Box sx={{ display: "flex", alignItems: "center" }} flexDirection={(mobile) ? "column" : "row"}>
                <Box sx={{
                    display: "flex", flexDirection: "column", width: "90%", marginTop: 10, marginBottom: 2,
                    overflow:""
